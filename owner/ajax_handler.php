...
switch ($action) {
    ...
    case 'process_sale':
        ...
        try {
            ...
            // 1. Insert into sales table without transaction_number
            $sales_stmt = $db->prepare(
                "INSERT INTO sales (payment_method, price, discount, cash_amount, change_amount, gcash_amount, gcash_account_name) 
                 VALUES (:pm, :price, :discount, :cash, :change, :gcash, :gcash_name)"
            );
            $sales_stmt->execute([
                ':pm' => $payment_method,
                ':price' => $total,
    ...
    case 'notify_owner':
        ...
        try {
            $stmt = $db->prepare("INSERT INTO notifications (recipient_role, message, created_at, is_read) VALUES ('owner', :message, NOW(), 0)");
    ...
    case 'get_notifications':
        try {
            $stmt = $db->prepare("SELECT id, message, created_at, is_read FROM notifications WHERE recipient_role = 'owner' ORDER BY created_at DESC LIMIT 50");
            $stmt->execute();
    ...
    case 'mark_notifications_read':
        try {
            $stmt = $db->prepare("UPDATE notifications SET is_read = 1 WHERE recipient_role = 'owner' AND is_read = 0");
            $stmt->execute();
            echo json_encode(['success' => true]);
        } catch (PDOException $e) {
            echo json_encode(['success' => false, 'message' => 'Database error: ' . $e->getMessage()]);
