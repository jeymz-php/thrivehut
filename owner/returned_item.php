    ...
    function showOrderedItemsTable(items, transactionId) {
        ...
        document.querySelectorAll('.return-btn').forEach(btn => {
            btn.onclick = function() {
                ...
                const tid = this.dataset.transaction;
                const qtyInput = this.parentElement.querySelector('.return-qty-input');
                ...
                const reason = prompt(`Enter reason for returning ${qty} of this item:`);
                if (reason && reason.trim() !== '') {
                    processReturn(pid, tid, qty, reason);
                }
            };
        });
    }

    function processReturn(productId, transactionId, quantity, reason) {
        fetch('ajax_handler.php', {
            method: 'POST',
        ...
        .then(data => {
            if (data.success) {
                ...
                // Optionally, refresh the modal or the main table
                document.getElementById('view-list-modal').style.display = 'none';
                location.reload();
            } else {
